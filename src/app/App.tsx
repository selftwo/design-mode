...
import type { AnnotationIntent, BoardDocument, EngineName, FrameElement, KitStateValue, ReviewAnnotation, ToolMode } from '@/features/review-board/model/board-document.schema'
...
export default function App() {
  ...
  const [learnLensAsking, setLearnLensAsking] = useState(false)
  const [learnLensAskError, setLearnLensAskError] = useState<string | null>(null)
  ...
  const askLearnQuestion = useCallback(async () => {
    ...
    try {
      ...
      setLearnLensAskError(error instanceof Error ? error.message : 'The teach question failed.')
      setLearnAnswer(null)
      setLearnAnswerRunId(null)
      setLearnPinnedElementId(null)
    } finally {
      setLearnLensAsking(false)
    }
    ...
    <div className="app-shell dm">
      ...
      >
        ...
        <LearnLensIsland
          ...
          answer={learnAnswer}
          asking={learnLensAsking}
          askError={learnLensAskError}
          canAsk
      ...
      <div className="status-banners" data-testid="status-banners">
        {localHost && hostConnection === 'reconnecting' ? (
          <p className="host-connection-notice dm-notice" data-tone="info" role="status" data-testid="host-connection-notice">
